...
const renderFiles = (content, contentName, onContentClick, currentFolder, currentFileIndex, showThumbnails, onRemoveClick) => {
    ...
    const renderNext = showThumbnails ? <div><div style={{marginLeft:'10px'}}>{content.name}</div><div>{content.created}</div><div>{content.geoCoordinateText}</div></div> : renderButton(<span>Next</span>, onClick(currentFileIndex+1))
        ...
        <tr key={content.name}>
          <td>
            ...
            <div style={removeStyle}>
                ...
                <span><button onClick={onRemoveClick.bind(null, contentName)}>Remove</button></span>
